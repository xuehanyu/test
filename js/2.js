 ...
 */
const POSTWHITELIST = ['SaveGreetings', 'SaveStaffInfo', 'ConfigRuleAuth', 'SaveManagerUserInfo']
...
*/
function post (url, params, errorCb) {
  ...
  return axios({
    ...
    transformRequest: [
      function (data) {
        ...
        // 如果参数值是数组的话，传递方式会特别点
        // [{xx:11}] => '[{xx:11}]'
        const getJsonArrayStr = arr => /\[.{0,}\]/.exec(JSON.stringify({ arr }))[0]
        const dataCopy = JSON.parse(JSON.stringify(data))
        Object.keys(dataCopy).forEach(key => {
    ...
    .then(res => {
      ...
      const downMethods = ['DownLoadFile']
